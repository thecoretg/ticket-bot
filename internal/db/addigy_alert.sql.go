...
const upsertAddigyAlert = `-- name: UpsertAddigyAlert :one
INSERT INTO addigy_alert (
    id,
    ticket_id,
    level,
...
type UpsertAddigyAlertParams struct {
	ID              string     `json:"id"`
	TicketID        *int       `json:"ticket_id"`
	Level           string     `json:"level"`
	Category        string     `json:"category"`
...
func (q *Queries) UpsertAddigyAlert(ctx context.Context, arg UpsertAddigyAlertParams) (*AddigyAlert, error) {
	row := q.db.QueryRow(ctx, upsertAddigyAlert,
		arg.ID,
		arg.TicketID,
		arg.Level,
		arg.Category,
