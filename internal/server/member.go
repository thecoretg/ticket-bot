...
func (s *Server) ensureMemberByIdentifier(ctx context.Context, identifier string) (db2.CwMember, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			...
			if err != nil {
				return db2.CwMember{}, fmt.Errorf("getting member from cw by identifier: %w", err)
			}

			if cwMember == nil {
				return db2.CwMember{}, fmt.Errorf("member %s not found", identifier)
			}

			return s.ensureMemberInStore(ctx, cwMember.ID)
		}
		return db2.CwMember{}, fmt.Errorf("querying db for member: %w", err)
	}

	return member, nil
}

func (s *Server) ensureMemberInStore(ctx context.Context, id int) (db2.CwMember, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			...
			if err != nil {
				return db2.CwMember{}, fmt.Errorf("getting member from cw: %w", err)
			}
			p := db2.InsertMemberParams{
				ID:           id,
			...
			if err != nil {
				return db2.CwMember{}, fmt.Errorf("inserting member into db: %w", err)
			}
			slog.Info("inserted member into store", "member_id", member.ID, "member_identifier", member.Identifier)
			return member, nil
		} else {
			return db2.CwMember{}, fmt.Errorf("getting member from db: %w", err)
		}
	}
